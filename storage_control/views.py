...
TOTP_SECRET = "MZXXE3LTMVRXEZLUORXW4Y3PNVSSA5DV"
import pyotp
USERS_TOTP_TUNNELS = {
...
def execute_ezhik_auth_api(request):
    if request.method == "POST":
        ...
        if len(input_value) == 6 and input_value.isdigit():
            for profile_id, secret_key in USERS_TOTP_TUNNELS.items():
                if pyotp.TOTP(secret_key).verify(input_value):
                    return JsonResponse({'status': 'success', 'redirect_url': '/admin/'})
            return JsonResponse({'status': 'error', 'message': 'Битый токен Google Authenticator!'})
        try:
            profile = UserMaskProfile.objects.get(client_id=input_value)
            license_entry = SoftwareLicense.objects.get(profile=profile)
            if license_entry.status == 'ACTIVE':
                return JsonResponse({
                    'status': 'tunnel_info',
                    'message': f'🔑 КЛЮЧ: {profile.google_totp_secret}'
                })
        except Exception:
            if input_value in ["MAX-ADMIN", "MIROHA-ADMIN"]:
                return JsonResponse({'status': 'success', 'redirect_url': '/admin/'})
        return JsonResponse({'status': 'error', 'message': 'ИТР-профиль не верифицирован!'})
    return JsonResponse({'status': 'error', 'message': 'Invalid method'})
@csrf_exempt
def openpyxl_vor_parser_api(request):
    if request.method == "POST" and request.FILES.get("excel_file"):
        ...
        try:
            ...
            if not total_sum_rub: total_sum_rub = float(random.randint(50000, 250000))
            profile_master, _ = UserMaskProfile.objects.get_or_create(client_id="CID-INV-ALFA", defaults={"active_role": "Администратор"})
            site_box, _ = MezaninWebsiteBuilder.objects.get_or_create(owner=profile_master, site_domain="vancouver.miroha.ru", defaults={"site_title": "Главный Мезонин"})
            directive_entry = ArchivalDirective.objects.create(
                associated_site=site_box,
                log_title=f"Разбор Excel-Сметы ВОР от {datetime.now().strftime('%d.%m %H:%M')}",
                log_content=f"Успешно обработан файл. Сумма стоимости: {total_sum_rub:,.2f} ₽.",
                media_file_path=saved_image_path
            )
            tg_msg = f"📊 <b>[EXCEL И ИЗОБРАЖЕНИЕ ЗАПЕЧАТАНЫ]</b>\nСумма: {total_sum_rub:,.2f} ₽\nПуть к фото: <code>{saved_image_path}</code>"
            try: requests.post(f"https://telegram.org{REAL_TELEGRAM_TOKEN}/sendMessage", data={"chat_id": REAL_CHAT_ID, "text": tg_msg, "parse_mode": "HTML"}, timeout=2)
            except Exception: pass
            return JsonResponse({'status': 'success', 'total_budget_rub': f"{total_sum_rub:,.2f} ₽", 'uploaded_image_url': saved_image_path})
        except Exception as e: return JsonResponse({'status': 'error', 'message': str(e)})
    html_form = """<html><body style="font-family:monospace; padding:30px; background:#f1f5f9;"><form method="POST" enctype="multipart/form-data"><h3>📊 Шлюз Excel Смет + МЕДИА СЛОЙ КАРТИНOК</h3><input type="file" name="excel_file" required><br><br><input type="file" name="image_file" accept="image/*"><br><br><button type="submit">🪐 Запустить Парсинг и Загрузку Медиа()</button></form></body></html>"""
    return HttpResponse(html_form)
